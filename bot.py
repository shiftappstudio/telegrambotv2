...
from telegram.ext import ApplicationBuilder, CallbackQueryHandler, ContextTypes, MessageHandler, filters, CallbackContext
...
SAFETY_CHECKER = (os.getenv('SAFETY_CHECKER', 'false').lower() == 'false')
    ...
    if photo is not None:
        ...
        with autocast("cuda"):
            image = pipe(prompt=[prompt],
                                    ...
                                    num_inference_steps=num_inference_steps).images[0]
    return image, seed
async def generate_and_send_photo(update: Update, context: CallbackContext) -> None:
...
async def generate_and_send_photo_from_photo(update: Update, context: CallbackContext) -> None:
    ...
    await context.bot.delete_message(chat_id=progress_msg.chat_id, message_id=progress_msg.message_id)
    await context.bot.send_photo(update.effective_user.id, image_to_bytes(im), caption=f'"{update.message.caption}" - Generated by BabyTestBoat.', reply_markup=get_try_again_markup(), reply_to_message_id=update.message.message_id)
