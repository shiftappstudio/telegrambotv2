...
from telegram.ext import ApplicationBuilder, CallbackQueryHandler, ContextTypes, MessageHandler,CommandHandler, CaptionHandler, filters
...
async def generate_and_send_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await context.bot.send_photo(update.effective_message.chat_id, image_to_bytes(im), caption=f'"{update.message.text}" - Generated by @{update.effective_user.username}  | @MyAnimeAI | myanime.ai', reply_markup=get_try_again_markup(), reply_to_message_id=update.message.message_id)
...
async def generate_and_send_photo_from_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    im, seed = generate_image(prompt=update.message.caption.replace("/mya",''), photo=photo)
    await context.bot.delete_message(chat_id=progress_msg.chat_id, message_id=progress_msg.message_id)
    print(update.effective_user)
    print(update.user) 
    await context.bot.send_photo(update.effective_message.chat.username, image_to_bytes(im), caption=f'"{update.message.caption}" - Generated by @{update.effective_user.username} | @MyAnimeAI | myanime.ai', reply_markup=get_try_again_markup(), reply_to_message_id=update.message.message_id)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await context.bot.delete_message(chat_id=progress_msg.chat_id, message_id=progress_msg.message_id)
    await context.bot.send_photo(update.effective_message.chat_id, image_to_bytes(im), caption=f'"{prompt}" Generated by @{update.effective_user.username} | @MyAnimeAI | myanime.ai', reply_markup=get_try_again_markup(), reply_to_message_id=replied_message.message_id)
app = ApplicationBuilder().token(TG_TOKEN).build()
app.add_handler(CommandHandler('mya', generate_and_send_photo))
app.add_handler(CaptionHandler('mya', generate_and_send_photo_from_photo))
